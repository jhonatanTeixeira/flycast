...
| 1.7 | Loop dominante do `SH4_TCB` (~40-73% do profile em kofnw/mbaa) é preenchimento de Store Queue (`_vmem_WriteMem32` por palavra, 0xE0000000-0xE3FFFFFF) | `core/rec-ARM64/rec_arm64.cpp` (`GenWriteMemory`/`GenWriteMemoryFast`) | ALTO→**diagnóstico confirmado, fix tentado falhou** | confirmado (achado), tentativa de correção **revertida** | Identificado via `perf`+gdb ao vivo (2026-09-14): jogos enchem 2 Store Queues (8 palavras cada) com `MOV.L` individuais antes do `PREF`, e cada palavra paga uma chamada `GenCallRuntime`→`_vmem_WriteMem32` completa (salto pra fora do buffer JIT e volta) pra fazer o equivalente a um único store num buffer fixo (`sq_buffer`, já mapeado como ponteiro direto — confirmado lendo `_vmem_MemInfo_ptr[0xE0]` ao vivo). **Confirmado com `cycles` vs `instructions` sampling que não é stall (fica mais dominante em instructions, não menos) — é trabalho real, volume alto.** **Tentativa de fix:** checagem inline (`addr>>26==0x38`) em `GenWriteMemoryFast` pra gravar direto em `sq_buffer`, pulando a chamada. **Resultado: piorou ~6% (`core_average` 12,07→12,76-12,84ms, 2 rodadas consistentes), não melhorou.** Causa provável: a checagem nova roda em TODO write de 32 bits (não só SQ), e writes comuns são muito mais numerosos que os de SQ — o imposto no caso comum superou a economia no caso raro-mas-quente. **Revertido.** Uma correção que só afeta os call sites que já precisam de rewrite (via `ngen_Rewrite`, sem custo no caminho comum) é mais provável de funcionar, mas exige mexer em infra de signal handler/code-patching mais arriscada — não tentada ainda. |
